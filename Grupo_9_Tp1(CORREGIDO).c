...
//Indica si es una vocal con acento o una ñ sin distinguir entre mayuscula y minuscula
bool acento_o_enie(char letra)
{
	bool valido=false;
	if ((letra>=(char)ascii('á') && letra<=(char)ascii('ñ')) || letra==(char)ascii('é') || letra==(char)ascii('Ñ') || letra==(char)ascii('Á') || letra==(char)ascii('É') || letra==(char)ascii('Í') || letra==(char)ascii('Ó') || letra==(char)ascii('Ú'))
		valido=true;

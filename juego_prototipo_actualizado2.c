...
void input(cadena letra_ingresada)
{
	printf("\n\n");
...
{
	int auxiliar=strlen(letras_ingresadas);
	if (!buscar_letra_ingresada_normal(letras_ingresadas,letra[0])){
		letras_ingresadas[auxiliar]=letra[0];
		letras_ingresadas[auxiliar+1]=' ';
		letras_ingresadas[auxiliar+2]='\0';	
	}
...
void modificar_palabra_mostrar(cadena palabra_mostrar,cadena palabra_secreta,char pista,cadena letra_ingresada)
{
	int posicion;
	
	for (posicion=0;posicion<strlen(palabra_secreta);posicion++)
	{
		if (palabra_secreta[posicion]==pista || palabra_secreta[posicion]==letra_ingresada[0])
			palabra_mostrar[posicion]=palabra_secreta[posicion];
	}
return;	
}
...
{
	bool encontrado=true;
	if (strstr(palabra_secreta,letra_ingresada)==0)
		encontrado=false;
...
/*bool buscar_letra(cadena palabra_secreta,cadena letra_ingresada)
{
	int posicion=0;
	bool encontrado=true;
	while (palabra_secreta[posicion]!=letra_ingresada[0] && palabra_secreta[posicion]!='\0')
		posicion++;

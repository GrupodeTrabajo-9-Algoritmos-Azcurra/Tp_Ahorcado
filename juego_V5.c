...
{
	while (strlen(letra)>1 || (int)letra[0]>(int)'Z' || (int)letra[0]<(int)'A'){
		printf("Por favor ingrese solo un caracter\n");
		input_string(letra);
	}
...
void partida()
{
    ...
    while (cantidad_partidas > 0){
    	printf("Palabra y despues pista\n");
    	input_string(palabra_secreta);             // ACA SE DEBE SOLICITAR UN VALOR ALEATORIO DE LAS PALABRAS DE LAS CATEGORIAS
    	input_string(pista);
    	system("@cls||clear");
